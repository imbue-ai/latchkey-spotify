...
const HEADLESS_BROWSER_CHANNELS = ['chrome', 'msedge', 'chromium'] as const;
...
export class SpotifyBrowserUnavailableError extends Error {
  constructor(cause: unknown) {
    super(
      'No browser could be launched to refresh the Spotify token ' +
        `(tried ${HEADLESS_BROWSER_CHANNELS.join(', ')}). ` +
        'Install Google Chrome, or run `latchkey ensure-browser` to download a browser.',
      { cause }
    );
 ...
 */
export function isLoggedInUrl(url: URL): boolean {
...
export function createSpotify(sdk: LatchkeySdk) {
  const {
  ...
  type SpotifySessionCredentialsData = zod.infer<typeof SpotifySessionCredentialsSchema>;

  class SpotifySessionCredentials implements ApiCredentials {
    static readonly objectType = 'spotifySession' as const;
    readonly objectType = SpotifySessionCredentials.objectType;
    ...
    injectIntoCurlCall(curlArguments: readonly string[]): Promise<readonly string[]> {
      if (this.accessToken === undefined) {
        throw new ApiCredentialsUsageError(
          'Spotify credentials hold no access token yet. One is minted from the session ' +
    ...
    static fromJSON(data: unknown): SpotifySessionCredentials {
      const parsed = SpotifySessionCredentialsSchema.parse(data);
      return new SpotifySessionCredentials(
  ...
  async function readSessionCookie(context: BrowserContext): Promise<string | null> {
  ...
  async function mintAccessToken(
  ...
  ): Promise<SpotifySessionCredentials> {
    let tokenResponse;
    try {
      [tokenResponse] = await Promise.all([
        page.waitForResponse(
          (response) => response.url().startsWith(TOKEN_URL_PREFIX) && response.status() === 200,
      ...
      if (error instanceof Error && sdk.isTimeoutError(error)) {
        throw new SpotifyTokenMintError(
          `The Spotify web player did not request a token within ${String(MINT_TIMEOUT_MS / 1000)} s. ` +
    ...
    if (!body.success) {
      throw new SpotifyTokenMintError(
        'The Spotify web player returned a token response in an unexpected shape.'
      );
    }
    if (body.data.isAnonymous === true) {
      throw new SpotifyTokenMintError(
        'The Spotify session cookie is no longer valid: the web player minted an anonymous ' +
    ...
    return new SpotifySessionCredentials(
      ...
      new Date(body.data.accessTokenExpirationTimestampMs).toISOString()
    );
  }
  ...
  class SpotifyServiceSession extends BrowserFollowupServiceSession {
    ...
    override readonly manualCredentialForm: ManualCredentialForm = {
      instructions:
        `To finish by hand, open ${WEB_PLAYER_URL} in the other tab of this window, then copy ` +
    ...
    override whileWaitingForLogin(page: Page): Promise<void> {
      this.noteUrl(page.url());
      return Promise.resolve();
    }

    private noteUrl(rawUrl: string): void {
      let url: URL;
      try {
        url = new URL(rawUrl);
      } catch {
    ...
    protected async performBrowserFollowup(context: BrowserContext): Promise<ApiCredentials> {
      ...
      if (sessionCookie === null) {
        throw new LoginFailedError(
          `The Spotify login left no ${SESSION_COOKIE_NAME} cookie behind.`
        );
      }
  ...
  class Spotify extends Service {
    ...
    readonly baseApiUrls = [WEB_PLAYER_API_BASE_URL] as const;
    ...
    override getCredentialsNoCurl(noCurlArguments: readonly string[]): ApiCredentials {
      const [sessionCookie, ...unexpected] = noCurlArguments;
      if (sessionCookie === undefined || unexpected.length > 0) {
        throw new SpotifyCredentialArgumentsError(
          `Expected exactly one argument, the value of the ${SESSION_COOKIE_NAME} cookie. ` +
